...
def plot_prediction(title, path_to_save, src, tgt, prediction, sensor_number, index_in, index_tar):
    ...
    plt.ylabel("OT (%)")
    plt.legend()
    plt.title("Forecast from ET " + str(sensor_number[0]))

    # save
    plt.savefig(path_to_save+f"Prediction_{title}.png")
    plt.close()

    def plot_training(epoch, path_to_save, src, sampled_src, prediction):
        ...
        plt.title("Teaching Forcing from ET Dataset , Epoch " + str(epoch))
        plt.xlabel("Time Elapsed")
        plt.ylabel("OT (%)")
        plt.legend()
